...
exports.listSentPackages = async (req, res) => {
  try {
    ...
    if (!clientId) {
      return res.json({ listPackages: [] });
    }
    let page = Number(req.body.page || 1);
    if (!page || page < 1) page = 1;
    const limit = 20;
    const offset = (page - 1) * limit;
    const where = ["packages.sender_id = ?"];
    ...
    const totalPages = Math.ceil(totalCount / limit);
    const [listPackages] = await db.query(
      `SELECT packages.id, packages.date_pack_start, packages.sender_id, c_sender.name AS senderName, c_sender.phone AS senderPhone, packages.receiver_id, c_receiver.name AS receiverName, c_receiver.phone AS receiverPhone, packages.sender_office_id, o_sender.office_name AS senderOffice, packages.receiver_office_id, o_receiver.office_name AS receiverOffice, packages.description, packages.weight, packages.homeSender, packages.homeReceiver, packages.status FROM packages LEFT JOIN clients AS c_sender ON packages.sender_id = c_sender.id LEFT JOIN clients AS c_receiver ON packages.receiver_id = c_receiver.id LEFT JOIN offices AS o_sender ON packages.sender_office_id = o_sender.id LEFT JOIN offices AS o_receiver ON packages.receiver_office_id = o_receiver.id WHERE ${whereSql} ORDER BY packages.date_pack_start DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
    res.json({ clientId, page, totalPages, listPackages });
  } catch (err) {
...
exports.listReceiverPackages = async (req, res) => {
  try {
    ...
    const limit = 20;
    const offset = (page - 1) * limit;
    const where = ["packages.receiver_id = ?"];
    const params = [clientId];
    if (req.body.dtStart) {
      where.push("packages.date_pack_start >= ?");
      params.push(toDateOnly(req.body.dtStart));
