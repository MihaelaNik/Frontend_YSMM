  ...
  try {
    ...
    const tableQueries = [
      ...
      CREATE TABLE IF NOT EXISTS users (
        ...
        user_type ENUM('client', 'employee', 'courier') DEFAULT 'client',
    ...
    // Update users table ENUM to include 'courier' if it doesn't already
    // Use a more robust approach: convert to VARCHAR temporarily, then back to ENUM
    try {
      ...
      if (tableCheck.length > 0) {
        const currentEnum = tableCheck[0].COLUMN_TYPE;
        // Check if 'courier' is already in the ENUM
        if (!currentEnum.includes("courier")) {
          console.log("Updating user_type ENUM to include 'courier'...");
          // Step 1: Convert to VARCHAR temporarily (preserves all data)
          await db.query(`
            ALTER TABLE users 
            MODIFY COLUMN user_type VARCHAR(20) DEFAULT 'client'
          `);
          // Step 2: Convert back to ENUM with all three values
          await db.query(`
            ALTER TABLE users 
            MODIFY COLUMN user_type ENUM('client', 'employee', 'courier') DEFAULT 'client'
          `);
          console.log("✅ Updated users table to include 'courier' in user_type ENUM");
        } else {
          console.log("✅ users table already has 'courier' in user_type ENUM");
        }
      }
    } catch (err) {
      console.log("⚠️  Could not update user_type ENUM:", err.message);
      // Try direct modification as fallback
      try {
        await db.query(`
          ALTER TABLE users 
          MODIFY COLUMN user_type ENUM('client', 'employee', 'courier') DEFAULT 'client'
        `);
        console.log("✅ Successfully updated user_type ENUM on retry");
      } catch (retryErr) {
...
console.log("5 employees seeded. Password for all: employee123");
    ...
    // Final check: ensure ENUM includes 'courier' before inserting
    // Use the same robust approach
    try {
      ...
      if (enumCheck.length > 0 && !enumCheck[0].COLUMN_TYPE.includes("courier")) {
        console.log("Final ENUM check: updating to include 'courier'...");
        // Convert to VARCHAR then back to ENUM
        await db.query(`ALTER TABLE users MODIFY COLUMN user_type VARCHAR(20) DEFAULT 'client'`);
        await db.query(`ALTER TABLE users MODIFY COLUMN user_type ENUM('client', 'employee', 'courier') DEFAULT 'client'`);
        console.log("✅ ENUM updated successfully");
      }
    } catch (alterErr) {
      console.log("Note: ENUM modification (final check):", alterErr.message);
    }
    
    await db.query(`DELETE FROM users WHERE user_type = 'courier'`);
    
    const courierPassHash = await bcrypt.hash("courier123", 10);
    ...
    try {
      await db.query(
        ...
        `,
        [
          "dimitar.stoyanov", "Димитър", "Стоянов", "dimitar.stoyanov@logistics.bg", "0888000006", courierPassHash,
          "svetoslav.georgiev", "Светослав", "Георгиев", "svetoslav.georgiev@logistics.bg", "0888000007", courierPassHash,
        ]
      );
      console.log("✅ 2 couriers seeded. Password for all: courier123");
    } catch (insertErr) {
      console.error("❌ Error inserting couriers:", insertErr.message);
      // If ENUM error, try one more time to fix it using the two-step approach
      if (insertErr.message.includes("Data truncated") || insertErr.message.includes("user_type")) {
        console.log("Attempting to fix ENUM issue using two-step conversion...");
        try {
          // Step 1: Convert to VARCHAR (preserves all data)
          await db.query(`ALTER TABLE users MODIFY COLUMN user_type VARCHAR(20) DEFAULT 'client'`);
          // Step 2: Convert back to ENUM with all three values
          await db.query(`ALTER TABLE users MODIFY COLUMN user_type ENUM('client', 'employee', 'courier') DEFAULT 'client'`);
          console.log("✅ ENUM fixed successfully, retrying courier insertion...");
          ...
          console.log("✅ 2 couriers seeded (after retry). Password for all: courier123");
        } catch (retryErr) {
          console.error("❌ Failed to seed couriers after retry:", retryErr.message);
          throw retryErr;
        }
      } else {
        throw insertErr;
